...
export function getCurrentItin(html, mptUsersettings) {
    ...
    let temp = exRE(
        html,
        /Carrier\s([\w]{2})\s([\w]+).*?Covers\s([\w()\s\-,]+)/g
    );
    ...
    for (let i = 0; i < temp.length; i += 3) {
        ...
        const currentDirtyFare = exRE(temp[i + 2], /(\w\w\w-\w\w\w)/g).map(
            l => `${l}-${next}-${current}`
        );
    ...
    for (let i = 0; i < segs.length; i += 15) {
        ...
        if (mptUsersettings.timeformat === "24h") {
            replacementsnew.push(
                (segs[i + 6].length === 4 ? "0" : "") + segs[i + 6]
            );
            replacementsnew.push(
                (segs[i + 7].length === 4 ? "0" : "") + segs[i + 7]
            );
        }
                        ...
                        for (
                            ...
                            if (
                                ...
                                for (var k = segnr; k <= j; k++) {
                                    itin[legnr]["seg"][k]["farebase"] =
                                        curfare[2];
                                    itin[legnr]["seg"][k]["farecarrier"] =
                                        curfare[3];
                                    dirtyFare[i] = curfare[2];
                                }
                                j = itin[legnr]["seg"].length;
                                ...
                                itin[legnr]["seg"][segnr + j]["farebase"] !== ""
                            ) {
