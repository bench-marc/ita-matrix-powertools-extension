...
export function getKlUrl(currentItin, edition, lang) {
    ...
    if (!pax) {
        console.error("Error: Failed to validate Passengers in printKL");
        return "";
    }
    ...
    klUrl += segs.map(
          seg =>
            `${seg.orig}:${seg.dep.year}${('0' + seg.dep.month).slice(-2)}${('0' + seg.dep.day).slice(-2)}@${('000' + seg.dep.time).slice(-4)}:${seg.carrier}${('000' + seg.fnr).slice(-4)}:${seg.bookingclass}>${seg.dest}`).join("-");
    klUrl +=
        "&ref=MS,fb=" +
        currentItin.farebases.join(".") +
        "&numberOfAdults=" +
        pax.adults +
        "&numberOfChildren=" +
        pax.children.length +
