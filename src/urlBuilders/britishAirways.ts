...
export function getBaUrl({ itin }: ICurrentItin, mptSettings: any, edition: string, language: string) {
    ...
    let url = 'https://www.britishairways.com/travel/fx/public/' + language + '_' + edition + '?eId=111054&data=F' + pax.adults + tmpPax.y + tmpPax.c + pax.infLap + 'LF';
    let mincabin = 3;
    ...
    for (let i = 0; i < itin.length; i++) {
        ...
        for (let j = 0; j < itin[i].seg.length; j++) {
            ...
            url += currentSegment.carrier + ('000' + currentSegment.fnr).slice(-4);
            url += cabins[currentSegment.cabin];
            url += currentSegment.orig + itin[i].seg[j + k].dest;
            if (itin.length == 2 && i == 1) {
                url += 'F';
            } else {
                url += 'T';
            }
    ...
    url += '&p=EUR6666.66&e=FP&c=' + cabins[mptSettings['cabin'] === 'Auto' ? mincabin : getForcedCabin(mptSettings)] + '&source=FareQuoteEmail&isEmailHBOFareQuote=false';
    return url;
}
