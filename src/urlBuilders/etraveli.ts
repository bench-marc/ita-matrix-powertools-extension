import { monthNumberToName, IDate } from '../utils';
...
function convertDate(date: IDate, withYear: boolean) {
    return ('0' + date.day).slice(-2) + monthNumberToName(date.month) + (withYear ? date.year.toString().slice(-2) : '');
}

export function getEtraveliUrl({ itin, numPax }: ICurrentItin, host: string) {
    if (itin.length > 2) return; // no multi segments
    if (itin.length == 2 && !(itin[0].orig == itin[1].dest && itin[0].dest == itin[1].orig)) {
        return; // no open jaws
    }
    ...
    ggUrl += itin[0].orig + itin[0].dest + convertDate(itin[0].dep, false);
    if (itin.length > 1) ggUrl += convertDate(itin[1].dep, false);
    ggUrl += '/' + numPax;
    ggUrl +=
        '?selectionKey=' +
        itin
            .map(function(itin) {
                return itin.seg
                    .map(function(seg) {
                        return seg.carrier + seg.fnr + '-' + convertDate(seg.dep, true) + '-' + seg.bookingclass;
                    })
                    .join('_');
            })
            .join('_');
